...
import { Music, X, FileAudio, Download, Volume2 } from 'lucide-react'
...
const MusicConverter = () => {
  ...
  const handleFileChange = (selectedFile: File) => {
    const allowedExtensions = ['mp3', 'm4a', 'aac', 'wav', 'flac']
    ...
    const interval = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 100) {
          ...
          const convertedSize = Math.floor(file.size * (selectedBitrate === '320 kbps' ? 1.2 : selectedBitrate === '256 kbps' ? 1.0 : selectedBitrate === '192 kbps' ? 0.8 : 0.6))
        ...
        {!file ? (
          <div 
            ...
            className={`border-2 border-dashed rounded-[20px] p-10 flex flex-col items-center justify-center gap-6 cursor-pointer min-h-[460px] transition-all duration-300 ${
              isDragging 
                ? 'border-violet-500 bg-violet-50/50 shadow-[0_0_15px_rgba(139,92,246,0.05)]' 
                : 'border-slate-200 bg-slate-50/50 hover:border-slate-350 hover:bg-slate-50'
            }`}
          >
            <div className="p-6 bg-white rounded-full border border-slate-100 text-violet-600 shadow-sm">
              <Music className="w-14 h-14" />
            </div>
            <div className="text-center">
              <p className="font-bold text-slate-800 text-lg">Drag & drop your audio here</p>
          ...
          <div className="flex flex-col gap-6 flex-grow">
            <div className="relative rounded-[16px] overflow-hidden bg-slate-100 border border-slate-250/60 p-8 flex flex-col items-center justify-center gap-4 shadow-inner">
              <div className="p-4 bg-violet-50 rounded-full border border-violet-100 text-violet-600 shadow-sm animate-pulse">
                <Volume2 className="w-10 h-10" />
              </div>
              <span className="font-semibold text-sm text-slate-600">Audio Track Loaded</span>
              {audioPreview && (
            ...
            <div className="bg-slate-50 border border-slate-150 rounded-2xl p-5 flex flex-col gap-4 text-sm">
              ...
              <div className="flex justify-between items-center py-1">
                ...
                <span className="text-violet-600 uppercase font-extrabold">{file.name.split('.').pop()}</span>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none py-3 px-4 rounded-xl border border-slate-200 bg-white text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all cursor-pointer shadow-sm pr-10"
              >
                {['Original', '320 kbps', '256 kbps', '192 kbps', '128 kbps'].map((br) => (
                  <option key={br.toLowerCase()} value={br.toLowerCase()}>
                    {br}
                  </option>
                ))}
              </select>
      ...
      <div className={`bg-white border border-slate-200 rounded-[24px] p-10 flex flex-col shadow-xl shadow-slate-100/50 transition-all duration-300 ${!outputFile ? 'opacity-60' : ''}`}>
        {!outputFile ? (
          <div className="border border-slate-150 rounded-xl bg-slate-50/40 p-8 flex flex-col items-center justify-center text-center flex-grow min-h-[460px]">
            <FileAudio className="w-14 h-14 text-slate-300 mb-3" />
            <p className="text-base font-semibold text-slate-400">Waiting for conversion...</p>
            <p className="text-sm text-slate-400/80 mt-2 max-w-[240px]">Your converted audio file will appear here for download.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-6 flex-grow">
            {/* Converted media preview */}
            <div className="relative rounded-[16px] overflow-hidden bg-slate-100 border border-slate-250/60 p-6 flex flex-col items-center justify-center gap-2 shadow-inner">
              <Volume2 className="w-10 h-10 text-emerald-500 animate-pulse" />
              <span className="font-semibold text-sm">Audio File Ready</span>
              {outputFile.url && (
                <audio src={outputFile.url} controls className="w-full mt-2" />
              )}
            </div>
